...
public class LandLossHistoricalImportDto
{
    ...
    // ===== 失地登记信息 =====
    @Excel(name = "征地时间", sort = 11, dateFormat = "yyyy-MM-dd")
    private String landRequisitionTime;

    @Excel(name = "完成补偿时间", sort = 12, dateFormat = "yyyy-MM-dd")
    private String compensationCompleteTime;

    @Excel(name = "认定时间", sort = 13, dateFormat = "yyyy-MM-dd")
    private String recognitionTime;

    @Excel(name = "征地批次", sort = 14)
    private String landRequisitionBatch;

    @Excel(name = "认定时所在村街", sort = 15)
    private String villageStreet;

    @Excel(name = "是否村合作经济组织成员", sort = 16, combo = {"是", "否"})
    private String isVillageCoopMember;

    @Excel(name = "登记备注", sort = 17)
    private String remark;

    // ===== 待遇核定（选填一组，有则整组必填） =====
    @Excel(name = "发放机构", sort = 18)
    private String grantOrg;

    @Excel(name = "开户名", sort = 19)
    private String accountName;

    @Excel(name = "与参保人关系", sort = 20)
    private String relationToInsured;

    @Excel(name = "银行账号", sort = 21)
    private String bankAccount;

    @Excel(name = "到龄年月", sort = 22, dateFormat = "yyyy-MM")
