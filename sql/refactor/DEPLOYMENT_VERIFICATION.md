...
> **文档状态（2026-08-02）**：本稿含历史设计/分析内容；基于旧表 `shebao_subsidy_distribution` 或旧「补贴发放」CRUD 的描述已过时。  
> **现行实现以 [`doc/`](../../doc/) 为准**，尤其 [`系统需求实现说明.md`](../../doc/系统需求实现说明.md)、[`补贴发放记录查询.md`](../../doc/补贴发放记录查询.md)、[`居民查询预发放与发放记录.md`](../../doc/居民查询预发放与发放记录.md)。
