...
CREATE TABLE IF NOT EXISTS `shebao_payment_plan` (
  ...
  `subsidy_type` varchar(50) DEFAULT NULL COMMENT '补贴类型(land_loss/expropriatee/demolition/village_official/teacher)',
  ...
  `distribution_status` varchar(20) DEFAULT NULL COMMENT '发放状态(pending待发放/submitted已提交银行/completed已完成)',
  ...
  KEY `idx_plan_subsidy_type` (`subsidy_type`),
...
CREATE TABLE IF NOT EXISTS `shebao_payment_plan_detail` (
  ...
  `distribution_result` varchar(20) DEFAULT NULL COMMENT '发放结果(success成功/failed失败)',
  `fail_reason` varchar(255) DEFAULT NULL COMMENT '发放失败原因',
  `del_flag` char(1) NOT NULL DEFAULT '0' COMMENT '删除标志(0正常 2删除)',
  `create_by` varchar(64) DEFAULT NULL,
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  `update_by` varchar(64) DEFAULT NULL,
