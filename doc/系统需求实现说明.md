...
shebao_benefit_notice_batch / detail  — 表结构存在，查询实现不落库
```
...
**落库**：实时查人员与登记子表；**不写入** `shebao_benefit_notice_batch/detail`（表脚本存在）。
...
**原因字典**：表字段/前端使用 `pause_reason`；初始化脚本中曾出现 `suspension_reason` 命名。
...
**已知边界**：明细保存时 `paymentMonth` 恒置 `null`；二次发放前端仍可选。
...
### 4.2 菜单权限与 Controller 差异（摘录）

| 功能 | 菜单脚本常见 perms | Controller `@PreAuthorize` |
|------|-------------------|------------------------------|
| 待遇暂停 | `shebao:benefit:suspension:list`（`04_menu_config.sql`） | `shebao:management:suspension:*` |
| 待遇恢复 | 脚本侧曾与暂停合并或缺失独立项 | `shebao:management:resume:*` |
| 核定 | `shebao:benefit:determination:*` | 与前端一致，使用 `benefit` 前缀 |

以运行库 `sys_menu` / 角色授权为准；初始化脚本与代码前缀不一致时会导致「有菜单无按钮」或 403。
...
| 基本信息修改两级 / 关键三级 | **已实现** |
| 预到龄通知导出 | **部分实现**（实时查询+导出；无批次落库/材料流程） |
