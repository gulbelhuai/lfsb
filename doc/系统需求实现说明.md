...
前端：`views/shebao/person/cancel`（动态菜单 + 静态路由 `person/cancel` 均已挂）。
...
```
shebao_payment_plan
  ├─1:N─ shebao_payment_plan_summary   (按补贴类型+村委会汇总)
...
完成发放 → FinanceAccountService.settleSubsidyDistribution（应发扣款 + 失败退回）
            → finance_account + 流水
```
...
- `determinationType=second`：现行定稿见 [`doc/二次发放需求.md`](二次发放需求.md)（本文旧「直接拒绝暂未实现」口径**废弃**）

**状态**：保存可为 `draft`；生成/提交为 `pending_review`。  
批次号：`yyyyMM` + `01/02` + 三位序号；插入后不再改。

**已知边界**：~~二次发放前端仍可选、后端拒绝~~（废弃，见定稿）。正常发放明细 `payment_month` 写入业务期 `yyyy-MM`（供银行附言等使用）。
...
| 财务审批→银行发放 | **已实现** |
| 二次发放 | **以定稿为准**（[`二次发放需求.md`](二次发放需求.md)；旧「接口拒绝」口径废弃） |
| 发放失败后更正/重发 | **失败处理页仅查询**；重发入口为支付计划「二次发放」（见定稿） |
| 发放记录查询 | **已实现**（只读支付计划明细） |
