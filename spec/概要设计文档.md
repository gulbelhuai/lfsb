...
- 可查看待审批的数据
- 可填写审批意见和退回原因
- 审批通过后进入财务环节,不直接执行银行动作
- 可逐级退回到上一节点

#### 2.2.4 财务人员

**权限范围**:
- 财务发放: 发放批次管理、银行文件导出、提交银行发放、导入发放结果
- 账户管理: 财务账号管理、账户余额查询、财务对账
- 失败处理: 发放失败反馈、二次/三次发放、人工处理

**操作边界**:
- 可查看待财务批次及其明细
- 当前业务通过 Excel 导出/导入完成银行结果确认,不包含银行直连
- 可导入银行发放结果并更新状态
- 可把失败结果反馈给业务人员进入更正和重发流程
...
| **待遇发放** | 3级审核 + 财务确认 | 草稿 → 待复核 → 待审批 → 待财务 → 已提交银行 → 已发放/发放失败 | 经办人 → 复核人 → 审批人 → 财务 |
...
| 5 | 待遇发放管理 | 支付计划生成、批次提交、复核、审批、按批次进入财务 | 5 | P0 |
| 6 | 财务管理 | 批次管理、银行文件导出/结果导入、失败处理、账户管理 | 5 | P1 |
...
**说明**:
- 支付工作的正式起点是“支付计划生成”,不是“提交银行发放”
- 财务环节当前采用 **Excel 导出/导入** 模式完成结果确认,不包含银行直连
- 当前代码基线中,支付计划、复核、审批、财务页面和接口已具备部分骨架,但提交银行、导入结果、失败重发等节点仍需继续补齐

**流程特点**:
| 特点 | 说明 |
|------|------|
| **逐级退回** | 支持从任意节点退回到上一节点 |
| **结果确认** | 通过导出银行 Excel、线下提交、导入结果形成财务确认闭环 |
| **失败重试** | 支持二次、三次发放(针对银行发放失败) |
| **统计展示** | 按补贴类型统计人数和金额,并按批次追溯成功/失败结果 |
...
| **支付结算** | 生成支付计划 | `shebao:payment:plan:*` | 生成支付计划、预览统计、查询明细 |
| | 支付批次创建 | `shebao:payment:batch:*` | 按所选支付记录创建批次并提交审核 |
...
| | 银行文件导出 | `shebao:finance:bank:file:*` | 导出银行 Excel 或银行格式文件 |
| | 提交银行发放 | `shebao:finance:bank:submit:*` | 提交银行发放 |
| | 导入发放结果 | `shebao:finance:bank:import:*` | 导入银行结果并回写批次、明细状态 |
...
| **失败处理** | 发放失败反馈 | `shebao:finance:failure:*` | 处理失败记录、反馈业务更正 |
| | 二次/三次发放 | `shebao:finance:failure:retry:*` | 发起或跟踪失败重发批次 |
