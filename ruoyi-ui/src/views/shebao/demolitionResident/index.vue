          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="拆迁事由" prop="demolitionReason">
                <el-input v-model="form.demolitionReason" placeholder="请输入拆迁事由" :disabled="subsidyFieldReadonly" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  computed: {
    ...
    selectedRowApproved() {
      ...
      const row = this.demolitionResidentList.find(item => item.id === id)
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      const target = row && row.id != null ? row : this.demolitionResidentList.find(item => item.id === id)
